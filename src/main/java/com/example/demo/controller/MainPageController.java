...
public class MainPageController {
    ...
    public ResponseEntity<String> uploadMedicationImg(@RequestPart("file") MultipartFile file,
        ...
        try {
            ...
            MedicationTake medicationTake = MedicationTake.builder()
                    ...
                    .medicationName(medicationSchedule.getMedication().getMedicationName())
                    .build();

            medicationTakeRepository.save(medicationTake); // DB에 저장
            user.getMedicationTakeList().add(medicationTake);
    ...
    public ResponseEntity<MedicationCalendarDto> getHealthcareMonthPage(@RequestParam(value = "date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)  LocalDate date){
        System.out.println("date = " + date);
        MedicationCalendarDto medicationCalendarDto = medicationService.requestHealthcareMonthPage(date);
        return ResponseEntity.ok(medicationCalendarDto);
    }
